...
class Views extends ClientAbstract {
	...
	public function find(array $params = array()) {
		...
		$endPoint = Http::prepare('views/'.$params['id'].'.json');
		$response = Http::send($this->client, $endPoint);
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
	...
	public function update(array $params) {
		...
		$response = Http::send($this->client, $endPoint, array ('view' => $params), 'PUT');
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
		return $response;
	...
	public function tickets(array $params = array()) {
		...
		$endPoint = Http::prepare('views/'.$params['id'].'/tickets.json');
		$response = Http::send($this->client, $endPoint);
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
	...
	public function count(array $params = array()) {
		...
		$endPoint = Http::prepare('views/'.(is_array($params['id']) ? 'count_many.json?ids='.implode(',', $params['id']) : $params['id'].'/count.json'));
		$response = Http::send($this->client, $endPoint);
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
	...
	public function export(array $params = array()) {
		...
		$endPoint = Http::prepare('views/'.$params['id'].'/export.json');
		$response = Http::send($this->client, $endPoint);
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
	...
	public function preview(array $params) {
		$endPoint = Http::prepare('views/preview.json');
		$response = Http::send($this->client, $endPoint, array('view' => $params), 'POST');
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
	...
	public function previewCount(array $params) {
		$endPoint = Http::prepare('views/preview/count.json');
		$response = Http::send($this->client, $endPoint, array('view' => $params), 'POST');
		if ((!is_object($response)) || ($this->client->getDebug()->lastResponseCode != 200)) {
			throw new ResponseException(__METHOD__);
		}
